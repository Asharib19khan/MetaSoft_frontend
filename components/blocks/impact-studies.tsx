    ...
    <section className="bg-base section-padding border-y border-line" id="impact">
      <div className="mx-auto w-full max-w-7xl px-5 lg:px-8">
        <div className="mb-16">
          ...
          <h2 className="font-display text-3xl md:text-4xl lg:text-6xl font-bold tracking-tight text-ink max-w-2xl leading-[1.1]">
          ...
          {CASES.map((item) => (
            ...
            >
              <div className="lg:w-1/3 flex flex-col pl-4 lg:pl-8 border-l-2 border-transparent transition-colors group-hover:border-ink z-10">
                <p className="font-display text-4xl md:text-5xl lg:text-7xl font-bold text-ink">{item.metric}</p>
              ...
              <div className="lg:w-2/3 flex flex-col lg:flex-row lg:items-start lg:items-center justify-between gap-6 lg:gap-12 mt-8 lg:mt-0 px-4 lg:px-8 z-10">
                <div className="flex flex-col max-w-xl">
                  <h3 className="font-display text-xl md:text-2xl lg:text-3xl font-bold text-ink mb-3 group-hover:underline decoration-1 underline-offset-4">{item.title}</h3>
                  <AnimatePresence>
                    {(hovered === item.id) && (
                      <motion.p 
